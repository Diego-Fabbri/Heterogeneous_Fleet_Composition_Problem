...
public class HFC_Model {
 ...
 //   this.n=n; //=v.length
   ...
   for(int k=0;k<p.length;k++){
    ...
    z = modello.intVar(1, 1, "z");// artificial variables whose values is 1
...
for(int k=0;k<v[0].length;k++){
   for(int t=0;t<v.length;t++){
   obj.addTerm(z, v[t][k]*c.get(2*k+v[0].length));// add c_H^k *v_{kt}*z
   obj.addTerm(x[t][k], -v[t][k]*c.get(2*k+v[0].length));// add -c_H^k *v_{kt}*x_t^k
   obj.addTerm(p[k],-c.get(2*k+v[0].length));// add -p_k*c_H^k 
   obj.addTerm(y[t][k],c.get(2*k+v[0].length));// add y_t^k*c_H^k
 }
}
